...
@SpringBootTest(classes = UpvoteApplication.class, webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT)
...
public class SpringIntegrationTest {
    ...
    protected static Response putMessage(String uri) {
        logger.info("Put url " + url + uri);
        return RestAssured.given().headers(getHeaders()).when().put(url + uri);
    }

    protected static Response getMessage(Map<String, Object> parametersMap, String uri) {
        logger.info("Get url " + url + uri);
        if (ObjectUtils.isEmpty(parametersMap)) {
